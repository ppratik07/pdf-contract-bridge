...
/**
 * Deploy contract to Ethereum Sepolia testnet
 */
export async function deployToEthereumSepolia(
  solidityCode: string,
  constructorArgs?: any[]
  ...
  try {
    ...
    console.log("Step 2: Connecting to Ethereum Sepolia testnet...");

    const rpcUrl = process.env.ETHEREUM_SEPOLIA_RPC_URL;
    const privateKey = process.env.DEPLOYER_PRIVATE_KEY;

    if (!rpcUrl) {
      console.error("Missing ETHEREUM_SEPOLIA_RPC_URL in .env");
      return {
        success: false,
        error: "Missing ETHEREUM_SEPOLIA_RPC_URL in environment variables",
      };
    }
